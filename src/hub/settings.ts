/**
 * /settings 页面 — 允许用户通过 app_token 验证身份后查看和修改高德 Key 配置
 */
...
export async function handleSettingsSave(
  ...
  if (amapKey) newConfig.amap_key = amapKey;
...
function renderConfigForm(
  ...
  const maskedKey = maskValue(cfg.amap_key || "");
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>高德地图 — 修改配置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>高德地图设置</h1>
    <p class="desc">修改您的高德 Key 配置</p>
    ...
    <form method="POST" action="/settings/save">
      ...
      <label>高德 Key</label>
      <input name="amap_key" type="password" placeholder="留空则不修改" />
      <p class="hint">在 <a href="https://lbs.amap.com/dev/key" target="_blank">高德开放平台</a> 获取</p>
      ...
      <div class="security-notice">
        ...
        <ul>
          ...
          <li>如需更高安全性，建议<a href="https://github.com/openilink/openilink-app-amap">自行部署</a>本 App</li>
        </ul>
      </div>
