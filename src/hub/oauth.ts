 ...
 *
 * 流程：
 * 1. 用户访问 /oauth/setup → 生成 PKCE 码对，重定向到 Hub 授权页
 * 2. Hub 回调 /oauth/redirect?code=xxx → 用 code + code_verifier 换取 token
 * 3. 将安装信息持久化到 Store
 */
...
/** 内存中暂存 PKCE state → verifier 映射（生产环境应使用 Redis 等） */
...
/**
 * 处理 OAuth 启动请求（/oauth/setup）
 * 生成 PKCE 码对并重定向到 Hub 授权页面
 */
...
/**
 * 处理 OAuth 回调请求（/oauth/redirect）
 * 用授权码 + code_verifier 换取访问令牌
 */
