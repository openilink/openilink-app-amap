...
function createTestTools() {
  const definitions: ToolDefinition[] = [
    { name: "geocode", description: "地址转坐标", command: "geocode" },
    { name: "get_weather", description: "天气查询", command: "get_weather" },
  ];

  const handlers = new Map<string, ToolHandler>();
  handlers.set("geocode", vi.fn().mockResolvedValue("坐标：116.481028,39.989643"));
  handlers.set("get_weather", vi.fn().mockResolvedValue("北京 晴 28°C"));
...
describe("Router", () => {
  describe("handleCommand", () => {
    it("正确路由到对应的工具处理函数", async () => {
      ...
      const event = makeCommandEvent("geocode", { address: "天安门" });
      const result = await router.handleCommand(event);

      expect(result).toBe("坐标：116.481028,39.989643");
      expect(handlers.get("geocode")).toHaveBeenCalledOnce();
    });
    ...
    it("传递正确的 ToolContext 给处理函数", async () => {
      ...
      const event = makeCommandEvent("geocode", { address: "故宫" });
      await router.handleCommand(event);

      const ctx = (handlers.get("geocode") as any).mock.calls[0][0];
      ...
      expect(ctx.args).toEqual({ address: "故宫" });
    });
    ...
    it("处理函数抛出异常时返回错误消息", async () => {
      ...
      const handlers = new Map<string, ToolHandler>();
      handlers.set("broken_tool", vi.fn().mockRejectedValue(new Error("高德 API 限流")));
  ...
  describe("getDefinitions", () => {
    it("返回所有已注册的工具定义", () => {
      ...
      expect(defs[0].name).toBe("geocode");
      expect(defs[1].name).toBe("get_weather");
    });
  });
